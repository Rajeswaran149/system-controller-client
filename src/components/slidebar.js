    ...
    return (
        <nav className={`h-full border-[2px] border-solid border-transparent rounded-md ease-in-out duration-700 ${isOpen ? "sm:w-72 w-[235px] absolute md:static bg-white md:bg-transparent" : "w-12"} overflow-hidden mr-3`}>
            <div className="flex items-center h-16 w-full border-b-2 border-solid border-[#D3D3D3]">
                <h2 className={`ease-in-out duration-700 text-amber-950 ${isOpen ? "w-52 h-full" : "w-0 h-full"}`} > 𝙀𝙈𝙎 </h2>
                <div
            ...
            <div className='h-full  overflow-y-scroll scrollbar-hide mt-3'>
                {menuItems.map((item, index) => (
                    <NavLink
                        ...
                        key={index}
                        className={({ isActive }) =>
                            `flex items-center whitespace-nowrap py-3 px-3 gap-4 mb-3 no-underline transition-all duration-500 ${isActive ? "bg-amber-950 text-white rounded-md" : "text-[#135049]"} hover:bg-amber-950 hover:text-white hover:rounded-md`
                        }
                    >
